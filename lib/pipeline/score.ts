...
import { RESUME_SUMMARY } from "./config";
...
async function scoreOne(job: Job, cfg: ScoreConfig, apiKey: string): Promise<Job> {
  ...
  const { score, reason } = parseScore(content);
  return { ...job, score, reason };
}

export async function scoreJobs(jobs: Job[], cfg: ScoreConfig): Promise<Job[]> {
  ...
  async function worker() {
    while (cursor < jobs.length) {
      const idx = cursor++;
      try {
        out[idx] = await scoreOne(jobs[idx], cfg, apiKey!);
      } catch (e) {
