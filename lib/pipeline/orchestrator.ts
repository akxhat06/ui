import { randomUUID } from "node:crypto";
  ...
  function emptyStep(name: StepName): StepRecord {                                                                                                                                                                   
    return { name, status: "idle", startedAt: null, finishedAt: null, durationMs: 0, count: 0 };                                                                                                                     
  }                                                                                                                                                                                                                  
  ...
  export async function runWorkflow(ctx: UserContext): Promise<RunRecord> {
    const existing = await getCurrent(ctx.userId);
    if (existing) return existing;                                                                                                                                                                                   
   
    const scrapeCfg = {                                                                                                                                                                                              
      ...SCRAPE_CONFIG,
      keywords:   ctx.techStacks.join(", "),                                                                                                                                                                         
    ...
    try {                                                                                                                                                                                                            
      const scraped = await runStep(ctx.userId, run, "scrape", async () => {
        const jobs = await scrapeJobs(scrapeCfg);                                                                                                                                                                    
        run.totals.scraped = jobs.length;                                                                                                                                                                            
        return { value: jobs, count: jobs.length };
      });                                                                                                                                                                                                            
                  
      const normalized = await runStep(ctx.userId, run, "normalize", async () => {                                                                                                                                   
        const seen = await loadSavedUrls(ctx.userId);
        const jobs = normalizeAndDedupe(scraped, seen);                                                                                                                                                              
        run.totals.afterDedupe = jobs.length;
        return { value: jobs, count: jobs.length, message: `${scraped.length - jobs.length} duplicates dropped` };                                                                                                   
      });                                                                                                                                                                                                            
                                                                                                                                                                                                                     
      const scored = await runStep(ctx.userId, run, "score", async () => {                                                                                                                                           
        if (normalized.length === 0) return { value: [] as Job[], count: 0 };
        const jobs = await scoreJobs(normalized, SCORE_CONFIG, ctx.resumeSummary);                                                                                                                                   
        run.totals.scored = jobs.length;                                                                                                                                                                             
        return { value: jobs, count: jobs.length };                                                                                                                                                                  
      });                                                                                                                                                                                                            
                  
      const passed = await runStep(ctx.userId, run, "filter", async () => {                                                                                                                                          
        const jobs = filterByScore(scored, SCORE_CONFIG.threshold);
        run.totals.passed = jobs.length;                                                                                                                                                                             
        return { value: jobs, count: jobs.length, message: `≥ ${SCORE_CONFIG.threshold}` };                                                                                                                          
      });                                                                                                                                                                                                            
                                                                                                                                                                                                                     
      await runStep(ctx.userId, run, "save", async () => {                                                                                                                                                           
        if (passed.length === 0) return { value: { added: 0, total: 0 }, count: 0, message: "nothing to save" };
        const result = await appendJobs(ctx.userId, passed);                                                                                                                                                         
        run.totals.saved = result.added;                                                                                                                                                                             
        run.xlsxPath = xlsxPathFor(ctx.userId);                                                                                                                                                                      
